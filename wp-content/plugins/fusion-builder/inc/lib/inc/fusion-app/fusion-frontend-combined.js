		...
		var fusionApp = Backbone.Model.extend( { // jshint ignore: line
			...
			initStudioPreview: function() {
				...
				jQuery( 'body' ).on( 'click', '.studio-wrapper .fusion-page-layout:not(.awb-demo-pages-layout) img', function( event ) {
					// TODO REMOVE RANDOM STRING FOR CACHE BUSTING
					var $item    = jQuery( event.currentTarget ).closest( '.fusion-page-layout' ),
						url      = $item.data( 'url' ) + '?=' + Math.random().toString( 36 ).replace( /[^a-z]+/g, '' ).substr( 0, 5 ),
...
function fusionMailchimpMapOption( $element ) {
	...
	// Cut off check.
	if ( 'object' !== typeof FusionApp.data.mailchimp || 'undefined' === typeof FusionApp.data.mailchimp.fields || 'undefined' === typeof FusionApp.data.fusionPageOptions.form_submission.fields.mailchimp_options.fields.mailchimp_lists ) {
		return;
	}
			...
			$selectField.each( function() {
				...
				$quickEditButton.on( 'click', function() {
					...
					if ( 'menu' === type ) {
						...
						window.open( fusionAppConfig.admin_url + 'post.php?post=' + itemValue + '&action=edit', '_blank' ).focus();
					}
				} );
