...
class AWB_Off_Canvas_Front_End extends AWB_Layout_Conditions {
	...
	private function __construct() {
		...
		add_action( 'wp_footer', [ $this, 'add_trigger_scripts' ], 99 ); // After jQuery is added in footer.
	...
	/**
	 * Add trigger scripts to footer.
	 *
	 * @since 3.7
	 * @access public
	 * @return void
	 */
	public function add_trigger_scripts() {

		// Nothing to do if WooCommerce is not active.
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$triggers = '';
		foreach ( self::$current as $item_id => $item_title ) {
			$options = self::get_options( $item_id );

			if ( 'yes' === $options['on_add_to_cart'] && self::is_current_user_can( $item_id ) && self::is_current_device_fit( $item_id ) ) {
				$triggers .= 'jQuery( function() {
					jQuery( document ).on( "added_to_cart", function() {
						if ( undefined !== typeof window.awbOffCanvas ){
							window.awbOffCanvas.open_off_canvas( ' . $item_id . ' );
						}
					} );
				} );';
			}
		...
		if ( '' !== $triggers ) {
			echo "\n" . '<script>' . "\n" . 'if ( "function" === typeof jQuery ) { ' . $triggers . ' }</script>' . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
